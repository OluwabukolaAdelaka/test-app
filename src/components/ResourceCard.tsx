...
export default function ResourceCard({ resource }: Props) {
  const getIconPath = (id: string) => {
    if (id === "1" || id === "3") return "src/assets/link-icon.svg";
    if (id === "2" || id === "4") return "src/assets/video-icon.svg";
    if (id === "5" || id === "6") return "src/assets/pdf-icon.svg";
    return "/assets/link-icon.svg";
  };

  const getSvgPath = (id: string) => {
    return `src/assets/card-${id}.svg`;
  };
